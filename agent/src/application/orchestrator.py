        ...
        if pools is None:
            ...
            for pool in pools:
                if "reserves" not in pool:
                    log_event(mode, "ERROR", "PARSE_ERROR", action="DISCARD_POOL", pool_id=pool.get("id"))
                    continue
                    
                score, reason = evaluate_pool(pool)
                if reason:
                    log_event(mode, "DEBUG", "POOL_DISQUALIFIED", pool_id=pool.get("id"), reason=reason)
                    continue
            ...
            if best_score < 1.5:
                log_event(mode, "INFO", "NO_ACTION", message=f"Best score {best_score} is below threshold. Doing nothing.")
            else:
                log_event(mode, "INFO", "DECISION_RECORDED", pool_id=best_pool_id, score=best_score, decision="ALLOCATE")
                if mode == "simulation":
                    log_event(mode, "INFO", "SIMULATION_END", message="[SIMULATION] Decision recorded. No transactions submitted.")
                    sys.exit(0)
                else:
                    submit_allocation(mode, best_pool_id, 500)
