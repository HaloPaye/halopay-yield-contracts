...
pub fn is_allowed(env: &Env, target: &Address) -> bool {
    env.storage().persistent().get(&DataKey::Allowlist(target.clone())).unwrap_or(false)
}

pub fn set_allowed(env: &Env, target: &Address) {
    env.storage().persistent().set(&DataKey::Allowlist(target.clone()), &true);
}
...
pub fn set_total_deposits(env: &Env, amount: u128) {
    env.storage().persistent().set(&DataKey::TotalDeposits, &amount);
}

pub fn get_total_allocated(env: &Env) -> u128 {
    env.storage().persistent().get(&DataKey::TotalAllocated).unwrap_or(0)
}

pub fn set_total_allocated(env: &Env, amount: u128) {
    env.storage().persistent().set(&DataKey::TotalAllocated, &amount);
}
